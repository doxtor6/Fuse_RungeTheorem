...
private lemma rectangle_cauchy_with_pole
  ...
  -- Strategy: split f(ζ)/(ζ-z) = dslope f z ζ + f(z)/(ζ-z) on each segment
  -- (segments avoid z since z is in the open interior).
  -- Then use Goursat for the holomorphic part and the explicit inverse integral.
  set A := (⟨x₀, y₀⟩ : ℂ) with hA
  set B := (⟨x₀ + s, y₀⟩ : ℂ) with hB
  set C := (⟨x₀ + s, y₀ + s⟩ : ℂ) with hC
  set D := (⟨x₀, y₀ + s⟩ : ℂ) with hD
  -- z is in U (closed rectangle ⊆ U, and Ioo ⊆ Icc).
  have hzU : z ∈ U := by
    apply hrect_sub_U
    · exact ⟨le_of_lt hz_int.1.1, le_of_lt hz_int.1.2⟩
    · exact ⟨le_of_lt hz_int.2.1, le_of_lt hz_int.2.2⟩
  have hf_diff_z : DifferentiableAt ℂ f z :=
    (hf.differentiableAt (hU.mem_nhds hzU))
  -- Properties of dslope f z.
  ...
  have hdslope_diff : DifferentiableOn ℂ (dslope f z) (U \ {z}) := by
    intro w hw
    have hwne : w ≠ z := hw.2
    have : DifferentiableAt ℂ (dslope f z) w := by
      have hw_in_U : w ∈ U := hw.1
      rw [show dslope f z = fun ζ => (ζ - z)⁻¹ • (f ζ - f z) from ?_]
      · -- differentiable at w because w ≠ z, sub is differentiable, etc.
        have h1 : DifferentiableAt ℂ (fun ζ : ℂ => (ζ - z)⁻¹) w := by
          apply DifferentiableAt.inv
          · exact (differentiableAt_id.sub_const z)
          · exact sub_ne_zero.mpr hwne
        have h2 : DifferentiableAt ℂ (fun ζ : ℂ => f ζ - f z) w :=
          (hf.differentiableAt (hU.mem_nhds hw_in_U)).sub_const (f z)
        exact h1.smul h2
      · funext ζ
        by_cases h : ζ = z
        · subst h
  ...
  -- Pointwise: on a segment that avoids z, f(ζ)/(ζ-z) = dslope f z ζ + f(z)/(ζ-z).
  have key_pointwise : ∀ ζ : ℂ, ζ ≠ z →
      f ζ / (ζ - z) = dslope f z ζ + f z / (ζ - z) := by
    intro ζ hζ
    rw [dslope_of_ne f hζ]
    -- slope f z ζ = (ζ - z)⁻¹ • (f ζ - f z)
    have hne : ζ - z ≠ 0 := sub_ne_zero.mpr hζ
    field_simp [slope_def_field]
  -- Segment integral linearity.
  have seg_lin : ∀ (a b : ℂ) (h₁ h₂ : ℂ → ℂ),
      IntervalIntegrable (fun t : ℝ => h₁ (a + (t : ℂ) * (b - a)) * (b - a))
        MeasureTheory.volume 0 1 →
      IntervalIntegrable (fun t : ℝ => h₂ (a + (t : ℂ) * (b - a)) * (b - a))
        MeasureTheory.volume 0 1 →
      segmentIntegral a b (fun ζ => h₁ ζ + h₂ ζ) =
        segmentIntegral a b h₁ + segmentIntegral a b h₂ := by
    intro a b h₁ h₂ hi₁ hi₂
    unfold segmentIntegral
    simp only [add_mul]
    exact intervalIntegral.integral_add hi₁ hi₂
  -- The segments are determined by these corner pairs. Let me show segments avoid z.
  -- Segment AB: bottom edge, im = y₀, but z.im > y₀, so ζ ≠ z.
  have hSeg_avoids_z : ∀ (a b : ℂ),
    -- Either same imaginary parts ≠ z.im or same real parts ≠ z.re
    ((a.im = b.im ∧ a.im ≠ z.im) ∨ (a.re = b.re ∧ a.re ≠ z.re)) →
    ∀ t : ℝ, a + (t : ℂ) * (b - a) ≠ z := by
    intro a b h t hcontra
    rcases h with ⟨him_eq, him_ne⟩ | ⟨hre_eq, hre_ne⟩
    · -- imaginary parts equal a.im = b.im ≠ z.im
      apply him_ne
      have : (a + (t : ℂ) * (b - a)).im = a.im := by
        simp [Complex.add_im, Complex.mul_im, Complex.sub_im, him_eq]
      rw [← this, hcontra]
    · apply hre_ne
      have : (a + (t : ℂ) * (b - a)).re = a.re := by
        simp [Complex.add_re, Complex.mul_re, Complex.sub_re, hre_eq]
      rw [← this, hcontra]
  -- Show each segment avoids z.
  ...
  have hDA_avoid : ∀ t : ℝ, D + (t : ℂ) * (A - D) ≠ z := by
    ...
    · simp [hD, hA]
    · simp [hD]
      exact ne_of_lt hz_int.1.1
  -- Now rewrite each segment integral.
  -- For each segment, f(ζ)/(ζ-z) integrand equals dslope f z ζ + f(z)/(ζ-z).
  have seg_eq : ∀ (a b : ℂ) (h_avoid : ∀ t : ℝ, a + (t : ℂ) * (b - a) ≠ z),
      segmentIntegral a b (fun ζ => f ζ / (ζ - z)) =
        segmentIntegral a b (dslope f z) +
          segmentIntegral a b (fun ζ => f z / (ζ - z)) := by
    intro a b h_avoid
    have eq_pointwise : ∀ t : ℝ,
        (fun ζ : ℂ => f ζ / (ζ - z)) (a + (t : ℂ) * (b - a)) * (b - a) =
        (dslope f z) (a + (t : ℂ) * (b - a)) * (b - a) +
        (fun ζ : ℂ => f z / (ζ - z)) (a + (t : ℂ) * (b - a)) * (b - a) := by
      intro t
    ...
    rw [show (fun t : ℝ =>
      (fun ζ : ℂ => f ζ / (ζ - z)) (a + (t : ℂ) * (b - a)) * (b - a)) =
        (fun t : ℝ =>
          (dslope f z) (a + (t : ℂ) * (b - a)) * (b - a) +
          (fun ζ : ℂ => f z / (ζ - z)) (a + (t : ℂ) * (b - a)) * (b - a)) by
          funext t; exact eq_pointwise t]
    -- Need integrability for the split.
    have hi₁ : IntervalIntegrable (fun t : ℝ =>
        (dslope f z) (a + (t : ℂ) * (b - a)) * (b - a))
        MeasureTheory.volume 0 1 := by
      -- continuity argument: dslope is continuous on U (proven above) but
      -- we need continuity of the parametrised function on [0,1].
      apply ContinuousOn.intervalIntegrable
      apply ContinuousOn.mul
      · -- We bound: continuous overall on ℝ since dslope is continuous on its full domain... no.
        -- Actually we use that dslope is continuous at every point on the segment.
        apply Continuous.continuousOn
        -- This needs dslope ∘ (parametrisation) to be continuous.
        -- Since dslope is generally not continuous everywhere, this fails.
        -- Use a different argument: continuous on a closed set.
        sorry
      · exact continuousOn_const
    have hi₂ : IntervalIntegrable (fun t : ℝ =>
        (fun ζ : ℂ => f z / (ζ - z)) (a + (t : ℂ) * (b - a)) * (b - a))
        MeasureTheory.volume 0 1 := by
      sorry
    exact intervalIntegral.integral_add hi₁ hi₂
  sorry
